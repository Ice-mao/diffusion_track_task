...
class TrackImageRunnerNoGym(BaseImageRunner):
    def __init__(self,
            output_dir,
            max_steps=200,
            n_obs_steps=4,
        ...
        self.env = auv_env.envs.world_auv_v2_sample.WorldAuvV2Sample(map="AUV_RGB_OpenWater_sonar",
                                                                    num_targets=1, 
                                                                    show=True)
        self.n_obs_steps = n_obs_steps
        self.n_action_steps = n_action_steps
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        # start rollout
        raw_obs, _ = env.reset()
        raw_obs = raw_obs['images']
        policy.reset()

        for i in range(10):
            raw_obs, reward, done, _, info = env.step(np.array([0 ,0]))
            raw_obs = raw_obs['images']
